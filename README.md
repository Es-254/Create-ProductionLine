...
| Source size / 工程规模 | `src/main/java` **57 Java files** / **~8,900 lines** (8,055 non-blank). The line count is a snapshot that moves with every commit; the file count is the stable part ／ **57 个 Java 文件 / 约 8,900 行**（非空 8,055 行）。行数每次提交都会变，文件数才是稳定值 |
...
| [`docs/qa.md`](docs/qa.md) | 无头自检 / QA | How to run the self test, the 21-check snapshot and the doc↔code consistency baseline ／ 自检运行方式、21 项快照、文档—代码一致性核对基线 |
