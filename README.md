...
| Source size / 工程规模 | `src/main/java` **57 Java files** / **~9,100 lines** (8,294 non-blank). The line count is a snapshot that moves with every commit; the file count is the stable part ／ **57 个 Java 文件 / 约 9,100 行**（非空 8,294 行）。行数每次提交都会变，文件数才是稳定值 |
...
| [`docs/qa.md`](docs/qa.md) | 无头自检 / QA | How to run the self test, the 22-check snapshot and the doc↔code consistency baseline ／ 自检运行方式、22 项快照、文档—代码一致性核对基线 |
