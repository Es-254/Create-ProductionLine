...
| Version / 版本 | **`1.0.3-snapshot.0.0.1` (alpha snapshot, in development)** / **`1.0.2` (release, current stable)** / **`1.0.1` (release, previous)**. The snapshot is the first look at the `1.0.3` line — the recipe-refresh rework — and ships as a **pre-release** (GitHub pre-release, CurseForge release type `alpha`, Modrinth channel *Alpha*); it is not a finished release. `1.0.2` adds the OP anvil flow plus the target-output / repeat budget and was **promoted from beta to a release on 2026-09-17**, so it stays the current stable release. Dev builds are **`0.0.0-dev.N`** (**beta**, built by `gradlew build -PdevBuild`, with N auto-incremented in `dev-build.txt`)。中文：**`1.0.3-snapshot.0.0.1`** 是 `1.0.3` 线的**alpha 快照**（配方刷新改造），按**预发布**发布（GitHub pre-release、CurseForge `alpha`、Modrinth *Alpha*），不是成品版；**`1.0.2` 仍是当前正式版**（2026-09-17 由 beta 晋升，含 OP 铁砧自定义流程与目标产量/循环次数）；`1.0.1` 为上一正式版；开发构建为 `0.0.0-dev.N`（beta），由 `-PdevBuild` 产出并按 `dev-build.txt` 递增。 |
