...
- **容器白名单**：`ClipboardCompat.isLoaderCarrier`（`compat/ClipboardCompat.java:83`）只认真 `LineSchemeItem`，加载柜槽位（`menu/SchemeLoaderMenu.java:54-55`）和 BE 收集路径（`block/entity/SchemeLoaderBlockEntity.java:220`，位于 `currentEntriesMap` 内；激活路径在 `:137` 也查一次）都走它。纸、剪贴板、镜像、随便伪造的 NBT 载体，一律拒收。
