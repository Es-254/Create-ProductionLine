...
| Dismantler / 破拆机 | 2 slots (item / optional scheme). An **unfinished intermediate** (Generic Intermediate + Create's `SEQUENCED_ASSEMBLY` component) is refunded by re-reading the sequence recipe it names: base + exactly the materials the finished steps consumed, plus a read-only mirror. A **finished product** refunds one full inverse batch (consume `count`, refund the inputs — including the product itself when the recipe consumes it, so `A + B = 2A` returns `A + B`), resolved server-side. That refund is a set of **unique materials, one each**, not one per recipe slot: `2 planks -> 4 sticks` hands back a single plank. That is deliberate — what keeps dismantling from being farmable is the batch rule (`consume >= count`), not the size of the refund — and an **unfinished intermediate is the exception**: it counts per deploy step, so every material the finished steps really consumed comes back. A **written Line Scheme** carries no materials at all (a plan costs one blank carrier to author), so dismantling one **erases the plan and hands back a fresh blank scheme**; a blank scheme and a mirror are refused with their own message. Tags refund their first registered member, fluid ingredients never can (they are counted and reported), and nothing is consumed when nothing can be refunded. | 2 格（物品 / 方案可选）。**未完成的中间产物**（通用中间产物 + Create `SEQUENCED_ASSEMBLY` 组件）会按它记录的序列配方**退还基底，外加已完成步骤真正吃掉的原料**，并给一份只读镜像；**成品**按"一次完整产出的逆运算"退款（消耗 count 个、退还输入——**含配方自己消耗的那份产物**，所以 `A + B = 2A` 退的是 `A + B`），配方由服务端解析。退的还是**去重后的原料集合**（每种一份、不是每个配方槽一份）：`2 木板 → 4 木棍` 只退回 **1 块木板**。这是有意为之——**防刷靠的是"整批消耗"规则**（`consume >= count`），不是退款份数；**未完成的中间产物是例外**：它按机械手步骤计数，已完成步骤真正吃掉的原料都会退回。**写好的产线方案本身不含任何原料**（做一份方案只花一个空白载体），所以拆方案就是**清空内容、返还一份空白方案**；空白方案与镜像各有专属提示、一律拒收。tag 退回首个成员，流体原料永远退不了（会统计并告知），退不出任何东西时不消耗物品。 |
...
4. **Dismantle / mirror** — the dismantler refunds materials (dropped beside the machine) and produces a read-only mirror; give it a **written Line Scheme** instead and it erases the plan, handing back a blank scheme. If it refuses, the reason arrives in your own chat.
...
4. **拆解/镜像**：破拆机还原原料（掉在机器旁）并生成只读镜像；放进去的是**写好的产线方案**时，则清空内容、返还一份空白方案。拒绝拆解时，原因会发到你自己的聊天框。
...
**EN** — An operator (permission level 2) can hand-author a Line Scheme in an anvil instead of computing one. Two cases block the computer, and both are about an item whose recipe the mod cannot turn into a Create line: an item **no recipe produces at all** (`RESULT_NO_RECIPE`), and an item whose live recipe **cannot be converted** — the milk bucket is the reported example, because Create fills it with a native process of its own (`RESULT_NOT_CONVERTIBLE`). In the first case a permission-level-2 requester gets a **placeholder** scheme written straight away; in the second the compute writes **nothing** and instead asks that player privately, in chat, whether to write one — the question keeps the reason ("this recipe cannot become a Create line") and adds two clickable answers. Answering **yes** writes the placeholder; **no** writes nothing. The question is bound to the player who pressed [Compute] and to the open menu, expires after **30 seconds**, and is dropped when the menu closes, when the target slot changes, or when that player computes again. A placeholder is the target, an **empty `RecipeId`** and zero steps — exactly the state the anvil calls "cleared", so a material can be hammered onto it straight away without spending paper first (the computer says so in its panel and chat, and the item's tooltip says it too). A placeholder installs **nothing**: the loader re-derives every recipe from `RecipeId`, and there is none, so the bar stays dark and the cabinet serves no recipe until the line is actually authored. For everybody else both computes are unchanged: the same result code, the same wording, an untouched carrier and no hint of a question. A scheme that already carries a plan, with `minecraft:paper` on the right, gives a **cleared** copy: every material step and every cached recipe payload is dropped, the target output and its per-craft count stay. From there, hammering the cleared scheme together with one material appends that material to an ordered list, and the plan — with its embedded native recipe — is rebuilt from that list. `paper` again **locks** the scheme, and a locked scheme refuses every later anvil operation. Two or more materials produce a `create:sequenced_assembly` line, the first material as the base and every later material as one Deployer step; exactly one material cannot be expressed that way, so the scheme is locked with a `single_material_fallback` flag, derivation picks the semantic single machine instead, and the game says `单原料自定义方案需等待后续版本支持` / "Single-material custom schemes are not supported yet" in the tooltip, once more in the action bar when it locks.
...
**中文** — OP（权限等级 2）可以不靠计算机，直接在铁砧里手写产线方案。有两种情况计算机会卡住，本质都是"这个物品的配方转不成 Create 产线"：某个物品**压根没有配方**（`RESULT_NO_RECIPE`），以及某个物品的实时配方**无法转换**——作者报的牛奶桶就是后者，因为 Create 用它自己的一套原生流程灌装（`RESULT_NOT_CONVERTIBLE`）。第一种情况下，权限等级 ≥ 2 的发起者会直接拿到一份**占位方案**；第二种情况下计算**什么都不写**，而是私下在聊天里问该玩家要不要写一份占位方案——提问里保留了原因（"该配方无法转成 Create 产线"），后面跟两个可点击的选项：选**写入**才会写占位方案，选**不写入**就什么都不写。提问只绑定按【计算】的那名玩家与当时打开的界面，**30 秒**后失效，界面关闭、目标格换物品或该玩家重新计算时都会作废。占位方案只记目标产物、`RecipeId` **为空**、零步骤——而这正好就是铁砧眼中的"已清空"状态，所以可以直接锤入原料，不必先浪费一张纸（计算机面板与聊天、物品 tooltip 都会这么说明）。占位方案**不会装任何配方**：加载柜的配方一律按 `RecipeId` 重新推导，而它是空的，所以存储条保持熄灭、柜子在产线真正写出来之前不提供任何配方。非 OP 玩家两条路径都完全不变：结果码、措辞、载体原样，也完全看不到这个提问。左槽放一份已经带方案的产线方案、右槽放 `minecraft:paper`，得到的是**清空**后的副本：材料步骤和内嵌配方缓存全部丢弃，目标产物和它的单次产出数量保留。之后每放一种原料敲一次即**锤入**：该原料追加进有序列表，方案连同内嵌的原生配方按这份列表重建。再放一次 `paper` 就把方案**锁定**，锁定后任何后续铁砧操作都会被拒绝。两种以上原料生成 `create:sequenced_assembly` 产线，第一种原料上带当基底、之后每种原料一台机械手；只有一种原料时铁砧表达不了序列装配，方案会带 `single_material_fallback` 标记锁定，推导改走单原料语义机器，并在锁定时提示`单原料自定义方案需等待后续版本支持` / "Single-material custom schemes are not supported yet"（tooltip 里常驻一条，锁定时再走一条 action bar 消息）。
